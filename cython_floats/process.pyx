...
def process(args):
    ...
    # return calculate(x_range, y_range, args.maxiter, args.ranges[2])
    return calculate(x_range, y_range, args.maxiter)


# cdef vector[string] calculate(vector[float] x_range, vector[float] y_range, int
#         maxiter, float scale):
cdef list calculate(vector[float] x_range, vector[float] y_range, int maxiter):
    ...
    for x in x_range:
        for y in y_range:
            iters = iterations_to_escape(x, y, maxiter)

            # data.push_back('%f %f %i' % (x, y, iters))
            # data.push_back('{:f} {:f} {}'.format(x/scale, y/scale, iters))
